...
//         // Vérifier la valeur du token avant de l'utiliser
//         console.log("Token from response:", token);
    
//         if (token) {
//           // Stocker le token dans les en-têtes de la requête Axios
//           axios.defaults.headers.common['Authorization'] = `Bearer ${token}`;
//           console.log("Token set in headers:", token); // Log du token dans les en-têtes
    
//           // Mettez à jour l'utilisateur dans le contexte d'authentification
//           setUser(user, role, token); // Ajoute le token à la fonction setUser
//           if (role === 'admin') {
//             navigate('/admin');
//           } else {
...
//           }
//         } else {
//           console.error("Token is undefined in response.");
//         }
//       }
//     } catch (error) {
//       setErrorMessage('Login failed. Please check your credentials.');
//       console.error("Login error:", error); // Log des erreurs de connexion
//     }
...
//   useEffect(() => {
//     const { name, email, password, cpassword } = errors;
//     setFormValid(!name && !email && !password && !cpassword);
//   }, [errors]);
...
//               <div className={`shadow-three mx-auto max-w-[500px] rounded  px-6 py-10  sm:p-[60px] ${isDarkMode ? 'bg-gray-800' : 'bg-white border-stroke'}`}>
...
//                     />
//                     {errors.cpassword && (
//                       <p className="text-red-500 text-sm mt-2">{errors.cpassword}</p>
//                     )}
//                   </div>
...
//                   </div>
//                 </form>
//                 <p className={`text-center text-base font-medium text-body-color ${isDarkMode ? 'text-gray-400' : ''}`}>
//                   Already using Startup?{" "}
//                   <Link to="/auth/signup" className="text-sky-700 hover:underline">
//                     Sign in
//                   </Link>
//                 </p>
//               </div>
...
const SignupPage = () => {
  ...
  const [formValid, setFormValid] = useState(false);
  const [errors, setErrors] = useState({ email: '', password: '' });
  // Déterminer si le dark mode est activé
  const isDarkMode = theme === 'dark';
  const { setUser } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation des champs
    const validationErrors = validate(name.value, email.value, password.value, cpassword.value);
    if (Object.keys(validationErrors).length > 0) {
      setErrors(validationErrors);
      setIsLoading(false);
      return;
    ...
    try {
      ...
      if (error.response.status === 422) {
        ...
        setErrors({
          ...
          cpassword: error.response.data.errors.password_confirmation?.[0] || '',
        });
      }
      setErrorMessage('Vos informations sont incorrectes. Veuillez réessayer.');
    } finally {
      setIsLoading(false);
    }
  };
